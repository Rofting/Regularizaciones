...
def _escribir_fila_gas(ws, fila: int, factura: dict, es_primera_del_bloque: bool):
    ...
    # Col O: precio m3 (fórmula =K/H si hay consumo m3)
    if f.get("consumo_m3"):
        ws[f"O{fila}"] = f"=K{fila}/H{fila}"

    # Col Q: precio kWh (fórmula =K/I si hay consumo kWh)
    if f.get("consumo_kwh"):
        ...
        try:
            ...
            if fila_bloque is None:
                ...
                if ultimo_fila == 0:
                    ...
                    fila_suma_ant = _encontrar_fila_suma(ws, ultimo_fila)
                    fila_bloque = (fila_suma_ant or 8) + 4
                ws.cell(row=fila_bloque, column=1).value = nombre_año
            ...
            # Para la pestaña de resumen, escribimos una fila de totales por fecha
            ...
            for fecha in fechas_unicas:
                ...
                if lecs_fecha[0].get("fecha_ant"):
                    ws[f"C{fila_datos}"] = _fecha_excel(lecs_fecha[0]["fecha_ant"])
                ...
                # Fila cuota fija
                cuota_fija_total = n_vecinos * precio_fijo_acs  # vecinos activos × cuota fija
    ...
    if lecturas_cal:
        try:
            ...
            for fecha in fechas_unicas:
                ...
                cuota_fija_total = n_vecinos * precio_fijo_cal
                ws[f"I{fila_datos}"] = cuota_fija_total
                ws[f"I{fila_datos}"].number_format = "#,##0.00"
                ws[f"J{fila_datos}"] = f"=SUM(H{fila_datos}:I{fila_datos})"
                fila_datos += 1
...
def crear_hoja_analisis(wb, nombre_año: str,
    ...
    for row in plantilla.iter_rows():
        for cell in row:
            ...
            if "LECTURAS ACS M3'!G" in v and fila_acs_suma_antigua is None:
                m = _re.search(r"ACS M3'!G(\d+)", v)
                if m:
            ...
            if "LECTURAS CALEF KWH'!G" in v and fila_calef_suma_antigua is None:
                m = _re.search(r"CALEF KWH'!G(\d+)", v)
                if m:
...
def actualizar_hoja_resumen(wb, nombre_año: str,
    ...
    import re as _re
    for row in ws.iter_rows(min_row=fila_cabecera, max_row=50):
        for cell in row:
            ...
            if cell.column == ultima_col and cell.value:
                ...
                if isinstance(cell.value, str) and cell.value.startswith("="):
                    # Actualizar referencias al ANALISIS anterior
                    analisis_anterior = [s for s in wb.sheetnames
                                        if s.startswith("ANALISIS")]
                    if len(analisis_anterior) >= 2:
                        viejo_analisis = analisis_anterior[-2]
                        v = v.replace(f"'{viejo_analisis}'", f"'{nombre_analisis}'")
                        v = v.replace(viejo_analisis, nombre_analisis)
                    # Actualizar refs a LECTURAS ACS M3 con las nuevas filas Suma
                    if "LECTURAS ACS M3" in v and filas_suma_lecturas.get("acs_suma"):
                        v = _re.sub(
                            r"'LECTURAS ACS M3'!([A-Z]+)(\d+)",
                            lambda m: f"'LECTURAS ACS M3'!{m.group(1)}{filas_suma_lecturas['acs_suma']}",
                            v
                        )
                    if "LECTURAS ACS M3" in v and filas_suma_lecturas.get("acs_ini"):
                        # La ref de periodo (días) apunta a la fila C del inicio del bloque
                        v = _re.sub(
...
if __name__ == "__main__":
    ...
    if not os.path.exists(RUTA_BD):
        print("❌ Ejecuta primero motor_reparto.py para cr